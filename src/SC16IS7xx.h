...
class SC16IS7xx: public Stream {
    public:
        SC16IS7xx(int uart_nr, spi_host_device_t host, gpio_num_t cs, gpio_num_t reset);
        SC16IS7xx(int uart_nr);

        void begin(const uint32_t baudrate, const uint32_t config=SC16IS7XX_SERIAL_8N1, unsigned long timeout_ms = 20000UL);
        void end();
        int read(void);
